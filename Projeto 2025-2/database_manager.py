...
def add_image(filename, file_path, capture_date, latitude, longitude, source):
    """Adiciona uma nova imagem ao banco de dados com status 'pending'."""
    try:
        conn = sqlite3.connect(DB_FILE)
        cursor = conn.cursor()
        sql = """
        INSERT INTO images (filename, file_path, capture_date, latitude, longitude, source)
        VALUES (?, ?, ?, ?, ?, ?);
        """
        cursor.execute(sql, (filename, file_path, capture_date, latitude, longitude, source))
        conn.commit()
        print(f"Imagem '{filename}' adicionada à fila para análise.")
        return cursor.lastrowid # Retorna o ID da imagem inserida
...
def get_pending_images():
    ...
    conn = sqlite3.connect(DB_FILE)
    # Row_factory faz com que o resultado seja um dicionário, mais fácil de usar
...
def remove_image(image_id):
    ...
    Usa o ID da imagem para encontrá-la.
    """
    conn = None
    try:
        conn = sqlite3.connect(DB_FILE)
        ...
        # 1. Primeiro, precisamos encontrar o caminho do arquivo para podermos apagá-lo.
        ...
        print(f"Registro da imagem ID {image_id} removido do banco de dados com sucesso.")

        # 3. Por último, apagamos o arquivo físico do disco.
        if os.path.exists(file_path_to_delete):
            os.remove(file_path_to_delete)
            print(f"Arquivo '{file_path_to_delete}' apagado do disco com sucesso.")
        else:
            print(f"Aviso: O arquivo '{file_path_to_delete}' não foi encontrado no disco.")
